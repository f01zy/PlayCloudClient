...
const Navigation = () => {
  ...
  return <nav className={styles.navigation}>
    ...
    <ul className={styles.links}>
      <li><div className={`${styles.input} ${searchInput ? styles.open : ""}`}><input type="text" onChange={onSearchChange} /><IoSearchSharp onClick={() => setSearchInput(!searchInput)} />{loading ? <Image src={"/circle-loader-white.svg"} className={loading ? "block" : "hidden"} alt="loading" width={100} height={100} /> : ""}</div><SearchResults /></li>
      {links.map(link => (
