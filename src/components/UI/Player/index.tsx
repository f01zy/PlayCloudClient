...
import { setMusicMode } from "@/store/music/music.slice"

const Player = () => {
  const { music, loading, mode } = useTypedSelector(selector => selector.musicSlice)
  ...
  const setMusicModeAndOnLoad = (mode: TMusicMode) => {
    ...
    const newMusic = mode === "all" ? user.history[Math.floor(Math.random() * (user.history.length - 1 - 0) + 0)] : music!
          ...
          <div className={styles.player}>
            ...
            <div className={styles.buttons}>
              ...
              <div className={styles.button} onClick={() => {
                handlePlayClick(dispatch, music, user, music.name, router, useTypedSelector)
              }}>
              ...
              <div className={styles.button} onClick={() => dispatch(setMusicMode(mode === "all" ? "one" : "all"))}>
                {mode === "one" ? <RiRepeatOneLine /> : <RiRepeat2Line />}
              </div>
            </div>
          </div>
        </>
