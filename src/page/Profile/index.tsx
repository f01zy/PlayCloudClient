...
const Profile: FC<{ id: string }> = ({ id }) => {
  ...
  return fetchUser ? (
    <div className={styles.profile}>
      ...
      {slide === ESlide.Playlists ? <div className={styles.tracks}>{fetchUser.playlists.map(playlist => playlist.author._id === user?._id && <div className={styles.track}><CardLittle key={playlist._id} {...playlist} /></div>)}</div> : ""}
    </div>
  ) : ""
}
