...
export const handlePlayClick = (dispatch: Dispatch<UnknownAction>, { _id, author, listenings, name, likes, date, type }: IMusic, user: IUser | null, musicName: string | undefined, router: AppRouterInstance) => {
  ...
  const { mode } = useTypedSelector(selector => selector.musicSlice)
  console.log(mode)
  if (musicName != name) { playerController.pause(); playMusic({ _id, author, name, listenings, likes, date, type }, dispatch, user); playerController.onEnded = () => { playMusic({ ...user.history[Math.floor(Math.random() * (user.history.length - 1 - 0) + 0)] }, dispatch, user) }; return }
