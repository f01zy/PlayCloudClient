...
export const handlePlayClick = (dispatch: Dispatch<UnknownAction>, { _id, author, listenings, name, likes, date, type }: IMusic, user: IUser | null, musicName: string | undefined, router: AppRouterInstance) => {
  ...
  if (musicName != name) { dispatch(setLoading(_id)); playMusic({ _id, author, name, listenings, likes, date, type }, dispatch, user); playerController.onEnded = () => { playMusic({ ...user.history[Math.floor(Math.random() * (user.history.length - 1 - 0) + 0)] }, dispatch, user) }; return }
