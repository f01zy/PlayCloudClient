...
export const handlePlayClick = (dispatch: Dispatch<UnknownAction>, { _id, author, listening, name, liked }: IMusic, user: IUser | null, musicName: string | undefined, router: AppRouterInstance) => {
  ...
  if (musicName != name) { playMusic({ _id, author, name, listening, liked }, dispatch, user); playerController.onEnded = () => { playMusic({ ...user.history[Math.floor(Math.random() * (user.history.length - 1 - 0) + 0)] }, dispatch, user) }; return }
